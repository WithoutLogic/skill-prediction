...
<FONT COLOR="#FFFFFF">debug [skill|loc|abnormal]</FONT> = Toggle debug modes.`)
	...
	function printPing() {
		...
		mod.command.message(`Ping=${ping.history.length ? `${ping.history[ping.history.length - 1]}, MinMax=(${ping.min}, ${ping.max}), Avg=${Math.round(ping.avg)}, Variance=${ping.max - ping.min}, Samples=${ping.history.length}` : '???'}`)
	}
